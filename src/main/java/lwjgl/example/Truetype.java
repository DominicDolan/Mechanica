...
import static lwjgl.example.util.IOUtil.*;
...
//        try {
            ttf = Res.INSTANCE.getFont().get("Roboto-Regular.ttf").getBuffer();
                    //ioResourceToByteBuffer("E:/Src/Mechanica/Git/mechanica/src/main/resources/res/fonts/RobotoMono-Regular.ttf", 512 * 1024);
//        } catch (IOException e) {
...
//        }
    ...
    private STBTTBakedChar.Buffer init(int BITMAP_W, int BITMAP_H) {
        ...
        STBTruetype.stbtt_BakeFontBitmap(ttf, getFontHeight() * getContentScaleY(), bitmap, BITMAP_W, BITMAP_H, 32, cdata);
    ...
    private void renderText(STBTTBakedChar.Buffer cdata, int BITMAP_W, int BITMAP_H) {
        float scale = STBTruetype.stbtt_ScaleForPixelHeight(info, getFontHeight());
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            for (int i = 0, to = text.length(); i < to; ) {
                ...
                float cpX = x.get(0);
                STBTruetype.stbtt_GetBakedQuad(cdata, BITMAP_W, BITMAP_H, cp - 32, x, y, q, true);
                x.put(0, scale(cpX, x.get(0), factorX));
                if (isKerningEnabled() && i < to) {
                    getCP(text, to, i, pCodePoint);
                    x.put(0, x.get(0) + STBTruetype.stbtt_GetCodepointKernAdvance(info, cp, pCodePoint.get(0)) * scale);
                }
