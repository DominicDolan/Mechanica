...
interface CircleDrawer {
    fun circle(x: Number = 0, y: Number = 0, radius: Number = 0.5f)

    fun circle(position: Vector, radius: Number = 0.5f) = circle(position.x, position.y, radius)
    fun circle(position: LightweightVector, radius: Number = 0.5f) = circle(position.x, position.y, radius)
